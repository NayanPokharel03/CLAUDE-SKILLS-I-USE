...
How to get the 126 skills in this folder working. Two very different targets — pick the section you need.
...
```
open-design/           77 skill folders + ABOUT.md
caveman/               20 skill folders + ABOUT.md
...
security-audit/         SKILL.md + ABOUT.md      <- itself a skill
```
...
- `karpathy-guidelines/` and `security-audit/` contain a `SKILL.md` directly. Copy the folder itself.
...
for r in superpowers ponytail ui-ux-pro-max karpathy-guidelines security-audit caveman; do
  if [ -f "$r/SKILL.md" ]; then cp -r "$r" "$S/$r"
  else for s in "$r"/*/; do cp -r "$s" "$S/$(basename "$s")"; done; fi
done
for s in open-design/*/; do cp -r "$s" "$S/$(basename "$s")"; done
```

## Verify

Every installed folder must contain a `SKILL.md`, and the count should be 126:
...
- **Context cost:** all 126 descriptions load into every session — roughly 10–15k tokens. If that's unwelcome, install only `superpowers`, `ponytail`, `karpathy-guidelines` and `caveman` at user level, and put `open-design` and `ui-ux-pro-max` in a project-level `.claude/skills/` where design work happens.
...
**Caveats:**
- One skill per upload. There is no bulk import — 126 uploads is not realistic, so pick the handful you actually use.
...
- Skills that shell out to scripts or CLIs (anything calling `.py`, `.sh`, `.mjs` or `.cjs` — `agent-browser`, `hatch-pet`, `web-clone`, the `brand/` and `design-system/` scripts) will **not** work in chat. They need a local machine. Only prose-and-reference skills transfer cleanly.

**Good candidates:** `ponytail`, `karpathy-guidelines`, `caveman`, `brainstorming`, `writing-plans`, `test-driven-development`, `systematic-debugging`, `surgical-patch`, `web-design-guidelines`, `taste-skill`.
